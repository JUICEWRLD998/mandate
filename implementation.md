...
> **STATUS: LIVE RUN COMPLETE 2026-09-03 — demo.sh Beats 0-5 exit 0 TWICE on testnet (16 PASS each).**
> D1 RESOLVED (live-verified): the cluster's profile schema is CLOSED to bank fields — user-upsert rejects
> legal_name/iban/swift_bic (`UnrecognizedKeys`); schema-backed markers (first_name/last_name/date_of_birth/
> verified_contacts.email.value) RESOLVE in-enclave from the OTP-bound user profile. Contract v0.3.0 (id 874,
> z:8e3547bc…:mandate-contracts) now: KYC = person markers; PAY = beneficiary from SEALED rail_beneficiary KV
> config + email marker. Three new platform findings verified (R15 loopback egress never allowed — enclave runs on
> the node; R16 modern member-delegation not enforced per-call — legacy agent-auth-update is the enforced surface
> (refines BUG-008 misattribution); R17 closed profile schema; R18 getAuditEvents TypeError in-session after a
> delegated execute). LIVE EVIDENCE: rail hits with real resolved values (Ada/1990-01-15; sealed beneficiary
> GB29… + email), iban_sha256 51374012… matches sha256(raw IBAN); revocation denial = Forbidden
> agent_auth_not_found (rail untouched). Rail must be PUBLIC (tunnel) — loopback unreachable from the enclave.
...
- Verify: `./scripts/demo.sh` runs green twice in a row (deterministic). — **VERIFIED LIVE 2026-09-03: Beats 0-5 exit 0 twice on testnet (16 PASS each); full evidence in the Phase 5 status block above.**
...
- [x] D1 marker strategy verified on live cluster 2026-09-03 (outcome + exact errors recorded: profile schema closed to bank fields — `UnrecognizedKeys { keys: [iban, legal_name, swift_bic] }`; schema-backed person/email markers resolve in-enclave; beneficiary via sealed rail_beneficiary KV — docs payroll model; see buglog BUG-010/012 + lib.rs marker-strategy note)
...
- [x] Deterministic demo (Beats 0–5) incl. revocation — LIVE GREEN 2026-09-03 (exit 0 twice; post-revoke denial = Forbidden `agent_auth_not_found`, rail untouched)
